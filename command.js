...
module.exports = {
    execute(message, client, cooldowns, prefix) {
        ...
        if (!command) return;
        if(command.disabled){if(message.author.id === '439039443744063488'){ } else { return message.reply(`Command is currently disabled, please try agian later.`) }}
        if(command.nsfw && !message.channel.nsfw) return;
        if(message.author.id === '439039443744063488'){
        } else {
            if (command.permissions) {
                const authorPerms = message.channel.permissionsFor(message.author);
                if (!authorPerms || !authorPerms.has(command.permissions)) {return message.reply(`Sorry, but you need to have the permission \`${command.permissions}\``)}
            }
